...
podman build --build-arg OSC_BUILD_VERSION=$commit --build-arg OSC_BUILD_COMMIT_SHA=${CI_COMMIT_SHA}  $LABEL --build-arg CI_SERVER_HOST=${CI_SERVER_HOST} --build-arg CI_PROJECT_PATH=${CI_PROJECT_PATH} --tag $docker_target:$commit --target gardener-extension-provider-ironcore -f Dockerfile .
